...
// ── name tag sprite ─────────────────────────────────────────────
export function makeNameTag(name: string, color: string, label: string, isKing: boolean, isPlayer: boolean): THREE.CanvasTexture {
  const c = canvas(256, 80);
  const g = c.getContext("2d")!;
  g.clearRect(0, 0, 256, 80);
  g.fillStyle = "rgba(16,20,26,0.78)";
  rr(g, 4, 8, 248, 62, 14);
  g.fill();
  g.strokeStyle = color;
  g.lineWidth = 4;
  rr(g, 4, 8, 248, 62, 14);
    g.stroke();
  g.fillStyle = isPlayer ? "#ffe066" : "#f2f4f8";
  g.font = '800 30px "Arial Black", system-ui, sans-serif';
  g.textAlign = "center";
  g.textBaseline = "middle";
  g.fillText(isKing ? `👑 ${name}` : name, 128, 32);
  g.fillStyle = color;
  g.font = '700 22px system-ui, sans-serif';
  g.fillText(label, 128, 58);
    return tex(c);
}
