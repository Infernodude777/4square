...
//    • Out of turn — hitting when it isnt your turn
//
...
export const WALL_HEIGHT  = 5.2;    // clear this and youre out
...
export const clamp = v: number, lo: number, hi: number) => Math.maxlo, Math.minhi, v));
...
export function createWState): WallState 
  return 
    ...
    opScore: 0,
        ...
        ballPos: new THREE.Vector30, 1.05, 4.5),
    ballVel: new THREE.Vector3),
    aimPos:  new THREE.Vector30, 0, 4.0),
    ...
    opPos: new THREE.Vector31.4, 0, 5.2),
    opY: 0,
    opVY: 0,
        opCrouch: false,
    opFacing: 0,
    opSwing: 9,
    opCooldown: 0,
    opTarget: new THREE.Vector31.4, 0, 5.2),
    opServeAt: 0,
...
// ── helpers ──────────────────────────────────────────────────
export function others: Side): Side  return s === player ? op : player; }

export function inCourtx: number, z: number): boolean 
  return Math.absx) <= COURT_HALF_W && z >= WALL_Z - 0.3 && z <= COURT_DEPTH;
}

/** Where will the ball next touch the blacktop? ballistic, ignores wall) */
export function predictLandingt: WallState):  x: number; z: number; time: number } 
   const  ballPos: p, ballVel: v } = t;
  const dy = p.y - GROUND_Y;
  const disc = v.y * v.y + 2 * GRAVITY * Math.max0, dy);
  const tt = v.y + Math.sqrtMath.max0, disc))) / GRAVITY;
...
export function callFoult: WallState, offender: Side, foul: Foul) 
  if t.phase === point || t.phase === won) return;
  t.phase = point;
  t.pointTimer = 2.0;
  t.shake = 0.55;
  t.banner = FOUL_TEXTfoul];
...
// ── Serve setup ──────────────────────────────────────────────
export function beginServet: WallState, who: Side) 
  t.server = who;
  t.turn   = who;
  t.held   = true;
  t.phase  = serve;
  ...
  t.ballPos.setholder.x, 1.05, holder.z - 0.35);
  ...
  * Launch the ball so it lands on the blacktop at bounceAt of the way to
   * the wall, then carries into the bricks. This guarantees the legal
    * ground-then-wall sequence for any well-struck ball.
     */
    function launcht: WallState, from: THREE.Vector3, def: ShotDef, quality: number) 
  const distToWall = Math.max1.0, from.z - WALL_Z);
  const defaultZ = clampfrom.z - distToWall * def.bounceAt, WALL_Z + 0.55, from.z - 0.3);
  ...
  // while a bad hit falls back toward the shots natural/default bounce.
  const desiredX = clampt.aimPos.x, -COURT_HALF_W + 0.45, COURT_HALF_W - 0.45);
  const desiredZ = clampt.aimPos.z, WALL_Z + 0.55, from.z - 0.3);
  ...
  const zControl =
    ...
    def.name === BOMB ? 0.70 :
    0.88;
      const xControl = Math.min1.55, def.lateral) * control;
  ...
  const targetX = clampxAim + spray, -COURT_HALF_W + 0.4, COURT_HALF_W - 0.4);
  const bounceZ = clampzAim, WALL_Z + 0.55, from.z - 0.3);

    const dx = targetX - from.x;
  const dz = bounceZ - from.z;
  const horiz = Math.hypotdx, dz) || 0.001;
  const speed  = def.speed * 0.65 + 0.35 * quality);
  const T = Math.max0.12, horiz / speed);
  ...
  if def.name === SLICE) t.ballVel.x += Math.signtargetX - from.x || 1) * 1.4 + quality * 1.1);
  if def.name === CROSS) t.ballVel.x += Math.signtargetX - from.x || 1) * 0.7 + quality * 0.7);
  ...
  t.ballPos.copyfrom);
  t.lastShot = kindOfdef);
}

/** Reverse-lookup a ShotKind from its definition small table, cheap). */
function kindOfdef: ShotDef): ShotKind 
   for const k in SHOTS) 
    if SHOTSk as ShotKind] === def) return k as ShotKind;
}
  return drive;
}
...
export function tryHit
  ...
  if t.phase === serve && t.held) 
    ...
    const kind = resolveShotKindcrouch, airborne, vy, finesse, shift);
        const def  = SHOTSkind];
    res.kind = kind;
    res.quality = 0.8 * execution;
    const from = new THREE.Vector3handPos.x, 1.0, handPos.z);
    launcht, from, def, res.quality);
    t.held = false;
        t.phase = live;
    t.turn = otherhitter);
    t.lastHitter = hitter;
  ...
  const dBall = handPos.distanceTot.ballPos);
  ...
  if t.bouncesAfterWall > 1)      res.foul = double; return res; }
    ...
    // ── Classify + timing
  let kind = resolveShotKindcrouch, airborne, vy, finesse, shift);
  if kind === smash && t.ballPos.y < 1.15) kind = drive;
  const def = SHOTSkind];
  res.kind = kind;

    const yErr  = Math.abst.ballPos.y - def.idealY);
  const yNorm = clamp1 - yErr / def.window, 0, 1);
  const yQ    = yNorm * yNorm * 3 - 2 * yNorm);
  const dQ    = clamp1 - dBall / HIT_REACH, 0.5, 1);
  const exec  = clampexecution, 0.3, 1);
    // Weighted toward being forgiving — even a rough hit returns cleanly.
  const q     = clamp0.55 * yQ + 0.45 * dQ) * exec, 0.30, 1);
  res.quality = q;
  res.perfect = yNorm > 0.72 && exec > 0.9;

    launcht, t.ballPos.clone), def, q);
...
// ── Physics step ─────────────────────────────────────────────
export function stepWallt: WallState, dt: number) 
  if t.phase === won) return;

    if t.phase === point) 
    t.pointTimer = Math.max0, t.pointTimer - dt);
    return;
}

  // Ball sits in the servers hand until they strike it.
  if t.phase === serve && t.held) 
  ...
  //    time it takes the ball to cross its own radius.
    const v = t.ballVel;
  const speed = Math.hypotv.x, v.y, v.z);
  const maxStep = BALL_R * 0.6;            // never move more than this per slice
  const slices = Math.max1, Math.min8, Math.ceilspeed * dt) / maxStep)));
  const h = dt / slices;

    for let i = 0; i < slices; i++) 
    if integratet, h)) return;           // a foul ended the rally
}
}

/** One integration slice. Returns true if a foul fired stop stepping). */
function integratet: WallState, dt: number): boolean 
   const b = t.ballPos;
  const v = t.ballVel;
  ...
  if b.y <= GROUND_Y && v.y < 0) 
    ...
    v.z *= 0.92;
    if v.y < 0.45) v.y = 0;
    sfx.bounceMath.min8, Math.absv.y) * 1.2 + 2));

    if t.hitWall) 
      t.bouncesAfterWall++;
      if t.bouncesAfterWall > 1)  callFoult, t.turn, double); return true; }
} else 
      t.bouncesBeforeWall++;
}

    if !inCourtb.x, b.z))  callFoult, t.lastHitter ?? t.turn, out); return true; }
}

  // ── Wall contact front brick face)
  if b.z <= WALL_Z + BALL_R && v.z < 0) 
    if b.y > WALL_HEIGHT)  callFoult, t.lastHitter ?? t.turn, long); return true; }
    if t.bouncesBeforeWall === 0)  callFoult, t.lastHitter ?? t.turn, short); return true; }
    b.z = WALL_Z + BALL_R;
    v.z = -v.z * WALL_BOUNCE;
    v.x *= 0.97;
        // Per-shot upward kick: roofers and moonballs balloon way up and back,
    // scrapies and baby hits stay low and short.
    v.y = v.y * 0.96 + SHOTSt.lastShot].wallKick;
    t.hitWall = true;
    t.bouncesAfterWall = 0;
    sfx.hit0.55);
}
  ...
  // If it hasnt bounced in court yet, the hitter shot wide fault on hitter).
  if Math.absb.x) > COURT_HALF_W + BALL_R) 
    if t.hitWall && t.bouncesAfterWall >= 1) 
          callFoult, t.turn, dead); return true;
} else 
      callFoult, t.lastHitter ?? t.turn, wide); return true;
}
}

  // ── Over the top of the wall
  if b.y > WALL_HEIGHT + 0.6 && b.z < WALL_Z + 2.2) 
    callFoult, t.lastHitter ?? t.turn, long); return true;
}
